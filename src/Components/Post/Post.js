...
function Post(props) {
    ...
    const prodImgPath = 'https://skills-share-server-production.up.railway.app/';
    
    return (
        <Paper className={CSSstyle.paper} elevation={3}>
        <Container style={{display: 'flex'}}>
        <Avatar variant="rounded" className={CSSstyle.avatar} src={`${prodImgPath}/images/uploads/${post && !user ? post?.postedBy?.avatar : user.avatar }`} />
        <Box>
